...
\setlist[itemize]{label={}, leftmargin=*, labelsep=3pt, itemsep=0pt, topsep=0pt, partopsep=0pt, parsep=0pt}
...
\clearpage
\vbox{
\head{Inter-Process Communication}
...
Effets des appels système :
\begin{itemize}
	\item \lstinline!fork()! : le fils hérite de tous les IPC de son père.
	\item \lstinline!exit()! / \lstinline!exec()! : Les accès aux objets IPC sont perdus, mais ATTENTION, les objets IPC n'ont pas été supprimés. Dans le cas de la mémoire partagée, le segment est détaché.
\end{itemize}
}
...
\vskip 5pt
\textbf{Segment de mémoire partagée} -- \lstinline!<sys/mman.h>!\\
\emph{Principe.} Zone de mémoire attachée à un processus, mais accessible pour d'autres processus. 
...
\func{key_t ftok(const char* path, int id); -- <sys/ipc.h> <sys/types.h>}
{
...
\func{int xxxget(key_t key, int flag); }
{
\begin{itemize}
...
\emph{Message} : composé de :
\begin{itemize}
	\item \lstinline!Type! : nombre entier ( long ) strictement positif.
	\item \lstinline!Donnee! : Suite d'octets contigus en mémoire.
\end{itemize}
\vspace{-5pt}\begin{lstlisting}
...
\emph{Socket (TCP / UDP / IP)} : Communication intra-tâches distante, bidirectionnelle, fiable sans perte de données.

\textbf{Types de sockets :}\\
\lstinline!-> SOCK_STREAM!
\begin{itemize}
	\item Mode connecté (fonctionnement similaire à un tube)
	\item avec contrôle de flux
	\item bidirectionnel
	\item protocole TCP (IPPROTO\_TCP)
\end{itemize}
\lstinline!-> SOCK_DGRAM!
\begin{itemize}
	\item Mode non connecté (possible d'inverser les paquets)
	\item transmission par paquets
	\item sans contrôle de flux
	\item bidirectionnel
	\item protocole UDP (IPPROTO\_UDP) ou IGMP (IPPROTO\_IGMP)
\end{itemize}
\lstinline!-> SOCK_SEQPACKET!
\begin{itemize}
	\item Mode non connecté garantissant l'intégrité
	\item transmission par paquet
	\item bidirectionnel
	\item protocole UDP ou IGMP
\end{itemize}
\lstinline!-> SOCK_RAM!
\begin{itemize}
	\item Accès direct avec la couche IP
	\item réservé au super-utilisateur
	\item protocole ICMP (IPPROTO\_ICMP)
	\item définition de nouveaux protocoles (IPPROTO\_RAW)
\end{itemize}

\textbf{Choix du protocole :}\\
\lstinline!-> TCP! (Transport Control Protocol)
\begin{itemize}
	\item Pas de perte, pas de déséquencement, flux
	\item Coûteux (connexion, transfert)
	\item Type = STREAM
\end{itemize}
\lstinline!-> UDP! (User Datagram Protocol)
\begin{itemize}
	\item Perte, déséquencement, taille limitée.
	\item Performant
	\item Type = DGRAM
\end{itemize}
...
struct sockaddr {
	unsigned short sa_family; /* Adress family, AF_XX */
	char sa_data[14]; /* 14bytes of protcol adress */
}; /* tous les pointeurs de structures d'adresse de socket seront castes a un pointeur de ce type avant d'etre utilises dans les differentes fonctions */
\end{lstlisting}
\func{int socket(int domaine, int type, int protocole);}{
\begin{itemize}
		\item \lstinline!domaine! : \lstinline!AF_UNIX | AF_LOCAL | AF_FILE | AF_INET | AF_INET6 | AF_UNSPEC!
		\item \lstinline!type! : \lstinline!SOCK_STREAM, SOCK_DGRAM, ...!
		\item \lstinline!protocole! : 0 (choix automatique) | \lstinline!IPPROTO_TCP! | \lstinline!TPPROTO_UDP!)
		\item @return : descripteur de fichier ou -1 en cas d'erreur
\end{itemize}
}
}
...
\textbf{Nommage :}\\
\emph{Objectif} : Associer un nom à une socket, pour identifier un serveur d'une manière unique.\\
\emph{Domaine unix} : nom=nom de fichier\\
\emph{Dom. internet (inet)} : nom=<numéro de port,adresse IP>\\
\func{int bind(int sock, struct sockaddr *nom, int lg_nom);}{
\begin{itemize}
	\item \lstinline!sock! : numéro de socket (retourné par socket())
	\item \lstinline!nom! : nom de la socket(dépendant du domaine)
	\item \lstinline!lg_nom! : longueur du nom \lstinline!sizeof(struct sockaddr)!
\end{itemize}
}

\textbf{Nommage dans le domaine unix (local)}\\
...
\textbf{Nommage dans le domaine Internet}
\vspace{-5pt}
\begin{lstlisting}
...
\textbf{Nommage : Formattage d'adresse}\\
\func{u_short htons(u_short)}{Host TO Network Short, conversion du numéro de port}
\func{u_long htonl(u_long)}{Host TO Network Long, conversion de l'adresse IP}
\func{u_short ntohs(u_short)}{Network TO Host Short}
\func{u_long ntohl(u_long)}{Network TO Host Long}
\func{char* inet_ntoa(struct in_addr adr)}{adr à chaine a.b.c.d (affichage)}
\func{u_long inet_addr(char* chaine)}{chaine à adr}

\textbf{Nommage : Correspondance nom/adresse}
\vspace{-5pt}
\begin{lstlisting}
#include <netdb.h>
struct addrinfo {
	...
	struct sockaddr *ai_addr; /*socket binary adress */
...
\vbox{
\vspace{-5pt}
\func{int getaddrinfo(const char *host, const char* serv, const struct addrinfo *hints, struct addrinfo **result);}{
...
\lstinline!host! : hostname ou adresse IP\\
\lstinline!serv! : numéro de port ( ou 0 )\\
\lstinline!hints! : si pas NULL spécifie les critères de sélection\\
\emph{@return} : 0 en cas de succès, errno sinon
}

\textbf{Nommage Internet : numéro de port}\\
...
\end{lstlisting}
Famille de fonctions de recherche d'infomation dans la services database \lstinline!/etc/services!, une connexion à la database est ouverte si nécessaire. :\\
\func{struct servent *getservbyname(char *name, char *proto);}{
Cherche l'entrée dans la database de nom \lstinline!name!, et de protocole \lstinline!proto!. Si \lstinline!proto = NULL! n'importe quel protocole sera cherché.\\
\emph{@return} : ptr sur servent, \lstinline!NULL! si erreur ou \lstinline!EOF!
}
...
Cherche l'entrée dans la database de port \lstinline!port!, Si \lstinline!proto = NULL! n'importe quel protocole sera cherché.\\
\emph{@return} : ptr sur servent, \lstinline!NULL! si erreur ou \lstinline!EOF!
}
\func{void setservent(int stayopen);}{
Ouvre une connexion à la database, et set l'entrée suivante comme première entrée. Si \lstinline!stayopen! n'est pas nul, la connexion à la database ne sera pas fermée entre deux appels à getserv*().
}
...
\vbox{
\textbf{Sockets non connectées : communication}\\
\func{int recvfrom(int sock, char* buffer, size_t length, int flags, struct sockaddr *adrsrc, socklen_t *adr_len);}{
Appel bloquant. Lecture d'un buffer addressé à la socket \lstinline!sock!, si \lstinline!adrsrc! n'est pas \lstinline!NULL! il prend l'adresse de l'émetteur, \lstinline!adr_len! est initialisé avec la taille du buffer associé à \lstinline!adrsrc! puis modifié au retour avec la taille de l'adresse.\\
\lstinline!flags! : ( 0: default )\\
\lstinline!- MSG_OOB! : données hors bande\\
\lstinline!- MSG_PEEK! : Lecture sans modification de la file d'attente\\
\lstinline!- MSG_WAITALL! : Lecture reste bloquante jusqu'à réception d'au moins \lstinline!length! octets\\
\emph{@return} : taille du message lu, 0: aucun message à lire ou déconnexion, -1 en cas d'erreur.
}
\func{size_t sendto(int socket, const void *buffer, size_t length, int flags, const struct sockaddr *dest_addr, socklen_t dest_len);}{
Envoie par la socket \lstinline!sock! du contenu de \lstinline!buffer! de taille \lstinline!length! à l'adresse \lstinline!dest_addr!
}

\textbf{Sockets connectées : Connexion}\\
\emph{Côté serveur :}\\
- 1 sock pour les demandes de connexion\\
- 1 sock pour les communications\\
\func{int listen(int sock, int taille_file);}{
Appel non bloquant. Création de la file d'attente des requêtes de connexion.\\
\emph{@return} : 0: succès, -1: erreur
}
\func{int accept(int sock, struct sockaddr *adr, socklen_t *adr_len);}{
Attente d'acceptation d'une connexion. Extrait la première requête de connexion dans la file, crée une nouvelle socket avec les même propriétées de \lstinline!sock!, et alloue un nouveau descripteur pour la socket. Si aucune requête n'est dans la file : Appel bloquant.\\
\lstinline!adr! : identité du client\\
\lstinline!sock! : créé avec \lstinline!socket()!\\
\emph{@return} : le descripteur de la socket, ou -1 en cas d'erreur.
}

\emph{Côté client :}\\
- 1 sock pour communiquer\\
\func{int connect(int sock, struct sockaddr *adr, socklen_t adr_len);}{
Appel bloquant. Demande d'une connexion.\\
\emph{@return} : 0:succès, -1:error.
}
...
\textbf{Sockets connectées : comunication}\\
Primitives UNIX :\\
\func{int read(int sock, char *buffer, int buf_len);}{}
\func{int write(int sock, char *buffer, int buf_len);}{}
Regroupement de plusieurs écritures/lectures :\\
\func{int recv(int sock, char *buff, int buf_len, int flag);}{}
\func{int send(int sock, char *buff, int buf_len, int flag);}{}
\func{int recvmsg(int sock, struct msghdr *msg, int flag);}{}
\func{int sendmsg(int sock, struct msghdr *msg, int flag);}{}
-> Même \lstinline!flag! que recvfrom.\\

\textbf{Sockets connectés: déconnexion}\\
\func{int shutdown(int sock, int how);}{
\lstinline!sock! : descripteur de la socket\\
\lstinline!how! : mode de déconnexion
\begin{itemize}
	\item 0 : réception désactivée
	\item 1 : émission désactivée
	\item 2 : émission et réception désactivées
\end{itemize}
\emph{@return} : 0= succès, -1= erreur\\
\emph{Note} : shutdown est censé être suivi d'un close.\\
}

} % END VBOX

\textbf{Communication connectée avacnée : Attente simultanée}\\
\lstinline!#include <sys/include.h>!\\
\func{int select(int nfds, fd_set *readfds, fd_set *writefds, fd_set *errorfds, struct timeval *timeout);}{
Attente simultanée sur les trois ensembles de descripteurs.\\
\lstinline!nfds! : numéro du plus grand descripteur +1\\
\lstinline!timeout! : temps max d'attente, NULL=bloqué indéfiniment.\\
\emph{@return} : nb de descripteurs prêts, 0= temps limite expiré, -1= erreur.
}
